...
namespace ProKabbadiClientLayer
{
    class ClinetLayer
    {
        ...
        {
            Console.WriteLine("display the Matches Details");
            foreach (Matches matches in businessLayer.DisplayMatches())
            {
                Console.Write(matches.Match_Id + " ");
                Console.Write(matches.Match_Date + " ");
                Console.Write(matches.First_Team_Id.Team_Id + " ");
                Console.Write(matches.Second_Team_Id.Team_Id + " ");
                Console.Write(matches.First_Team_Score + " ");
                Console.Write(matches.Second_Team_Score + " ");
                Console.WriteLine();
              

            }
            BusinessLayer.exportExcelData();

        }

            public static void exportTextData()
            {
                string filePath = @"D:\Pro_Kabaddi.txt";
                using (StreamWriter writer = new StreamWriter(filePath))
                {
                    foreach (Teams teams in businessLayer.DisplayTeams())
                    {
                        writer.Write(teams.Team_Id + " ");
                        writer.Write(teams.Team_Name + " ");
                        writer.Write(teams.Team_City + " ");
                        writer.WriteLine();
                    }
                }
            }

        public static void FetchMatches()
        {
            Console.WriteLine("eneter the team name");
            string Team_Name = Console.ReadLine();
            Matches match = new Matches();
            
            BusinessLayer businessLayer = new BusinessLayer();
            List<Matches> matches2 = businessLayer.FetchMatches(Team_Name);
            matches2.Sort(match);
           // matches2.Reverse();

            foreach (Matches matches in businessLayer.FetchMatches(Team_Name))
            {
                Console.Write(matches.First_Team_Id.Team_Id);
                Console.Write(matches.Second_Team_Id.Team_Id);
                Console.Write(matches.Match_Date);
